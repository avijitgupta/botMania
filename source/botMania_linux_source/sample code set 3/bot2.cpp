...
while(1)
{
	...
	if(itaken!=1 && itaken_my!=1) //If nobody has taken flag, move towards flag
	{
		ang=atan2(y-myy,x-myx);//enemy flag trace
		for(int i=0;i<3;i++)
	        move(RAD(ang),1);
	} 
	else if(itaken!=1 && itaken_my==1)//enemy flag is there but ours gone
	{
         if(ti_my!=tid)
         {
	...
	if(ex[0]!=999)
	{
		...
		move(RAD(ang),1); //Turn towards enemy to fire
                xdis= ex[0]-myx;
                ydis= ey[0]-myy;
                distance= pow((xdis*xdis+ydis*ydis),0.5);
		if(bombcount>0)
